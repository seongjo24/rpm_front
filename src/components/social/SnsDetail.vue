...
<script>
  ...
  export default {
    ...
    created(){
      ...
      let url = this.$store.state.contents.context + 'loadBoard/' + this.boardSeq
      axios.get(url)
              .then(res=>{
                this.board = res.data
    ...
    methods:{
      ...
      checkThumb(){
        let url = this.$store.state.contents.context + 'thumbed/' + this.boardSeq + '/' + this.$store.state.user.user.userid
        axios.get(url)
                .then((res)=>{
                  if(res.data){
                    this.empty=false
      ...
      thumbUp(){
        if(this.$store.state.user.auth===false){
          ...
          this.empty=false
          this.fall=true
          let url = this.$store.state.contents.context + 'thumbUp/' + this.boardSeq + '/' + this.$store.state.user.user.userid
          axios.get(url)
                  .then(res=>{
                    if(res.data){
      ...
      thumbDown(){
        this.empty=true
        this.fall=false
        let url = this.$store.state.contents.context + 'thumbDown/' + this.boardSeq + '/' + this.$store.state.user.user.userid
        axios.get(url)
                .then(res=>{
                  if(res.data){
                    this.board.thumbCount -=1
